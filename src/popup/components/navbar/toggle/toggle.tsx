...
const Toggle: React.FC<{
  ...
  collapsed: boolean;
  darkMode?: boolean;
}> = ({ close, expand, collapse, collapsed = false, darkMode = false }) => (
    ...
    <motion.button whileTap={{ scale: 0.96 }} onClick={close} type="button">
      <svg width="20" height="20" viewBox="0 0 20 20">
        <g fill="none" fillRule="evenodd">
          <circle fill={darkMode ? '#31436C' : '#F5F5F7'} cx="10" cy="10" r="10" />
          <path
            fill={darkMode ? '#FFFFFF' : '#434D5A'}
            fillRule="nonzero"
